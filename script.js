...
function setupStartTimeWindow(selectedSections) {
  let meetingStartData = [];
  // Iterate through subjects
  selectedSections.forEach((subject) => {
    // Iterate through courses
    subject.forEach((course) => {
      // Iterate through sections
      if (course.sections.length > 0) {
        course.sections.forEach((section) => {
          // If all section meetings have the same start time
          if (section.meetings) {
            if (
              section.meetings.every(
                (meeting) =>
                  meeting.beginDate.split(' ')[1] ==
                  section.meetings[0].beginDate.split(' ')[1]
              )
            ) {
              // Push single point of data to the array
              meetingStartData.push({
                name: course.name,
                departmentCode: `${course.subjectCode.code}-${course.subjectCode.school}`,
                departmentName:
                  subjectData[course.subjectCode.school][
                    course.subjectCode.code
                  ].name,
                sectionCode: section.code,
                startTime:
                  parseInt(
                    course.sections[0].meetings[0].beginDate
                      .split(' ')[1]
                      .split(':')[0]
                  ) +
                  parseInt(
              ...
              section.meetings.forEach((meeting) => {
                meetingStartData.push({
                  ...
                  sectionCode: section.code,
                  startTime: meeting.beginDate.split(' ')[1],
                });
              });
            }
          }
  ...
  let width = document.getElementById('startTimeWrapper').offsetWidth;
  let height = document.getElementById('startTimeWrapper').offsetHeight;

  let xScale = d3.scaleTime().range([0, width]);

  let yScale = d3.scaleBand().rangeRound([0, height]).padding(0.2);

  const container = d3.select('#startTimeWrapper');

  container
    .append('g')
    .selectAll('dot')
    .data(meetingStartData)
    .enter()
    .append('circle')
    .attr('cx', (d) => d.startTime * (width / 24))
    .attr('cy', 20)
    .attr('r', 10)
    .style('fill', 'red');
}
